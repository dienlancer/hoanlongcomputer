...
class Frontend{	
	...
	public function add_css_file(){				
		wp_register_style('product_css',PLUGIN_URL . "public/frontend/css/product.css",array(),'1.0',false);
	...
	public function load_template($templates){
		...
		if(get_query_var('za_category') != ''){
			$file = PLUGIN_PATH . DS . "templates" . DS . "frontend" . DS . 'za-category.php';
			if(file_exists($file)){
				return $file;
			}			
		}
		if(get_query_var('zaproduct') != ''){
			$file = PLUGIN_PATH . DS . "templates" . DS . "frontend" . DS . 'zaproduct.php';
			if(file_exists($file)){			
				return $file;
			}
		}
